...
from apps.wish.wish_app import (check_user_wish_data, get_user_event_wish,
                                get_user_weapon_wish, get_user_wish_overview)
...
class WishCog(commands.GroupCog, name='wish'):
    ...
    async def wish_history(self, i: Interaction, member: Member = None):
        ...
        for index, tuple in enumerate(user_wish_history):
            ...
            wish_time = (datetime.strptime(
                tuple[3], "%Y/%m/%d %H:%M:%S")).strftime("%Y/%m/%d")
            wish_type = tuple[4]
    ...
    async def wish_analysis(self, i: Interaction, member: Member = None):
        ...
        player_luck = str(round(100*player.luck_evaluate(get_num=up_five_star_num,
                          use_pull=use_pull, left_pull=left_pull), 2))
        guarantee = text_map.get(370, i.locale, user_locale) if up_guarantee == 1 else text_map.get(
            371, i.locale, user_locale)

        embed = default_embed(message=f'• {text_map.get(373, i.locale, user_locale)} **{player_luck}%** {text_map.get(374, i.locale, user_locale)}\n'
    ...
    async def wish_char(self, i: Interaction, num: int):
        ...
        embed = default_embed(message=f'• {text_map.get(382, i.locale, user_locale)} **{num}** {text_map.get(383, i.locale, user_locale)}\n'
                              f'• {text_map.get(380, i.locale, user_locale)} **{left_pull}** {text_map.get(381, i.locale, user_locale)}\n'
                              f'• {guarantee}\n'
    ...
    async def wish_weapon(self, i: Interaction, item_num: int):
        ...
        if last_name == '':
            return await i.response.send_message(embed=error_embed().set_author(name=text_map.get(405, i.locale, user_locale), icon_url=i.user.avatar), ephemeral=True)

        view = ChooseWeapon.View(self.bot.db, i.user, i.locale, user_locale)
        embed = default_embed(message=f'{text_map.get(391, i.locale, user_locale)}:\n'
                              f'**{last_name}**\n'
        ...
        embed = default_embed(message=f'• {text_map.get(394, i.locale, user_locale)} **{item_num}** {text_map.get(395, i.locale, user_locale)}\n'
    ...
    async def wish_overview(self, i: Interaction, member: Optional[Member] = None):
        ...
        banner_names = [text_map.get(398, i.locale, user_locale), text_map.get(
            399, i.locale, user_locale), text_map.get(400, i.locale, user_locale)]
        total = 0
        for banner in overview.values():
            total += banner['total']
        embed = default_embed(
            message=f"{text_map.get(375, i.locale, user_locale)} {total} {text_map.get(376,i.locale, user_locale)}\n"
            f"{text_map.get(396, i.locale, user_locale)} {160*total} <:primo:958555698596290570> {text_map.get(397, i.locale, user_locale)}")

        for index, banner in enumerate(overview.values()):
            average = (banner['total'] // banner['five_star']
                       ) if banner['five_star'] != 0 else 0
            embed.add_field(
                name=f'{banner_names[index]}',
                value=f'• {text_map.get(375, i.locale, user_locale)} **{banner["total"]}** {text_map.get(376, i.locale, user_locale)}\n({banner["total"]*160} <:primo:958555698596290570> {text_map.get(397, i.locale, user_locale)})\n'
