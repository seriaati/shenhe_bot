...
from data.game.GO_modes import hit_mode_texts, infusion_aura_texts, reaction_mode_texts
...
class DamageCalculator:
    ...
    async def calculate_damage(self) -> Tuple[Dict, str, str]:
        ...
        if (
            self.current_character.id == 10000060
            and self.current_character.constellations_unlocked == 6
        ):  # yelan is C6
        ...
        await page.focus(
            "div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiCard-root.css-1kbwkqu > div.MuiCardContent-root.css-nph2fg:nth-child(2) > textarea.MuiBox-root.css-xkq1iw:nth-child(3)"
        )
        # write in the json data
        await page.keyboard.sendCharacter(str(good_json))
        # replace database button
        await page.click(
            "div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiCard-root.css-1kbwkqu > div.MuiCardContent-root.css-2s1u6n:nth-child(4) > button.MuiButton-root.MuiButton-contained.MuiButton-containedSuccess.MuiButton-sizeMedium.MuiButton-containedSizeMedium.MuiButtonBase-root.css-1p356oi"
        )
        ...
        await page.waitForSelector(
            "div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiMenu-paper.MuiPaper-elevation8.MuiPopover-paper.css-ifhuam:nth-child(3) > ul.MuiList-root.MuiList-padding.MuiMenu-list.css-1ymv12a > li.MuiMenuItem-root.MuiMenuItem-gutters.MuiButtonBase-root.css-szd4wn:nth-child(2)"
        )
        langauge_num = to_go(self.locale)
        await page.click(
            f"div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiMenu-paper.MuiPaper-elevation8.MuiPopover-paper.css-ifhuam:nth-child(3) > ul.MuiList-root.MuiList-padding.MuiMenu-list.css-1ymv12a > li.MuiMenuItem-root.MuiMenuItem-gutters.MuiButtonBase-root.css-szd4wn:nth-child({langauge_num})"
        )

        # go to the character's overview page
        await page.goto(
            f"https://frzyc.github.io/genshin-optimizer/#/characters/{character_name}"
        )
        # wait until the first label appears
        await page.waitForSelector(
            "div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiCard-root.css-1vbu9gx:nth-child(3) > div.MuiBox-root.css-q2n2qf:nth-child(1) > span.MuiTypography-root.MuiTypography-caption.css-5twvz:nth-child(3) > span.css-103d8rc"
        )
        # get all the labels
        ...
        for talent in talents_to_calculate:
            ...
            talent_name = await page.querySelector(
                f"div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiCard-root.css-1vbu9gx:nth-child({card_index}) > div.MuiBox-root.css-q2n2qf:nth-child(1) > h6.MuiTypography-root.MuiTypography-subtitle1.css-8ysn2z:nth-child(2) > span"
            )
            talent_name = await (
                await talent_name.getProperty("textContent")
            ).jsonValue()
            ...
            damage_rows = await page.querySelectorAll(
                f"div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiCard-root.css-1vbu9gx:nth-child({card_index}) > ul.MuiList-root.MuiList-padding.css-1jrq055:nth-child(3) > li.MuiListItem-root.MuiListItem-gutters.MuiListItem-padding.MuiBox-root.css-1n74xce"
            )
            index = 1
            for row in damage_rows:
                damage = await page.querySelector(
                    f"div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation0.MuiCard-root.css-1vbu9gx:nth-child({card_index}) > ul.MuiList-root.MuiList-padding.css-1jrq055:nth-child(3) > li.MuiListItem-root.MuiListItem-gutters.MuiListItem-padding.MuiBox-root.css-1n74xce:nth-child({index})"
                )
    ...
    async def convert_to_GOOD_format(self) -> Tuple[str, str, str]:
        ...
        for character in self.data.characters:
            burst_index = (
                3 if character.id == 10000041 or character.id == 10000002 else 2
            )
            talent = {
            ...
            if character.id == self.current_character.id:
                ...
                for index, member_id in enumerate(character_team):
                    if member_id == "":
                        character_team[index] = text_map.get_character_name(
                            self.current_character.id, "en-US"
                        ).replace(" ", "")
                        break
            character_team.sort(reverse=True)

            # get conditionals
            conditional, desc, eff = conditionals.get(character, self.current_character)
            if (str(character.id) == self.character_id) or (
                str(character.id) in self.team
            ):
                description += desc
                effect += eff
            ...
            weapon_key = (
                "TheCatch"
                if weapon.detail.name == '"The Catch"'
                else weapon.detail.name.replace("'", "")
                .title()
                .replace(" ", "")
                .replace("-", "")
            )
            good_dict["weapons"].append(
                {
                    ...
                    "location": (
                        text_map.get_character_name(character.id, "en-US")
                    ).replace(" ", ""),
                    "lock": True,
                }
            )
            for artifact in filter(
                ...
                for substat in artifact.detail.substats:
                    substats.append(
                        {"key": good_stats.get(substat.prop_id), "value": substat.value}
                    )
                good_dict["artifacts"].append(
                    {
                        "setKey": artifact.detail.artifact_name_set.replace("'", "")
                        .title()
                        .replace(" ", "")
                        .replace("-", ""),
                        ...
                        "slotKey": "plume"
                        if artifact.detail.artifact_type.name.lower() == "feather"
                        else artifact.detail.artifact_type.name.lower(),
                        "mainStatKey": good_stats.get(
                            artifact.detail.mainstats.prop_id
                        ),
                        "substats": substats,
                        "location": (
                            text_map.get_character_name(character.id, "en-US")
    ...
    def parse_damage_embed(
        ...
        infusion_str = (
            ...
            if self.infusion_aura != ""
            else ""
        )
        reaction_mode_str = (
            f"({text_map.get(reaction_mode_texts[self.reaction_mode], self.locale)})"
        ...
        for talent, damages in damage_dict.items():
            ...
            embed.add_field(name=talent, value=value, inline=False)
        conditions = ""
        if len(self.team) != 0:
            team_str = ""
            ...
            embed.add_field(
                name=text_map.get(345, self.locale), value=team_str, inline=False
            )
        if description != "":
...
class Conditional:
    ...
    def get_team(
        ...
        current_character: CharacterInfo,
    ):
        result = {}
        ...
        for character in team_characters:
            ...
            result[character.name.replace(" ", "")] = (
                {character.name.replace(" ", ""): conditional}
                | artifact_conditional
                | weapon_conditional
            )
    ...
    def get_artifact(
        self, character: CharacterInfo, current_character: CharacterInfo
        ...
        for artifact in filter(
            ...
            artifact_set_name = (
                artifact.detail.artifact_name_set.replace("'", "")
                .title()
                .replace(" ", "")
                .replace("-", "")
            )
                ...
                for conditional in self.artifact_conditionals:
                    if conditional["name"] == set_name:
                        result[conditional["name"]] = {
                            conditional["key"]
                            .replace(
                                "custom_element", current_character_element
                            ): conditional["value"]
                            .replace("custom_element", current_character_element)
                        }
                        description += f'• {conditional["description"]}\n'
                        effect += f"• {conditional['effect']}\n"
    ...
    def get(
        ...
        for conditional in self.conditionals:
            if conditional["name"] == character.name.replace(" ", ""):
                if (
                    (
                        conditional["ascension"] is not None
                        and int(character.ascension) >= int(conditional["ascension"])
                    )
                    or (
                        conditional["constellation"] is not None
                        and int(character.constellations_unlocked)
                        >= int(conditional["constellation"])
                    )
                    or (
                        conditional["ascension"] is None
                    ...
                    result[
                        conditional["key"].replace(
                            "custom_element",
                            str(current_character.element.name).lower(),
                        )
                    ] = conditional["value"].replace(
                        "custom_element", str(current_character.element.name).lower()
                    )
