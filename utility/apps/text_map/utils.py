...
def getCityName(city_id: int, locale: Locale, user_locale: Literal["str", None]) -> str:
    cities = {
        1: text_map.get(129, locale, user_locale),
        2: text_map.get(130, locale, user_locale),
        3: text_map.get(131, locale, user_locale)
    }
    return cities.get(city_id)


def get_weekday_name(day_num: int, locale: Locale, user_locale: Literal["str", None], full_name: bool = False) -> str:
    if not full_name:
        weekday_dict = {
...
def get_element_name(element_name: str, locale: Locale, user_locale: Literal["str", None]) -> str:
    element_dict = {
    ...
    return element_dict.get(element_name) or element_dict.get(element_name.lower()) or element_name
...
async def get_user_locale(user_id: int, db: aiosqlite.Connection) -> Literal["str", None]:
    c = await db.cursor()
