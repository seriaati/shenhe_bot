...
class CharacterSelect(Select):
    def __init__(self, options: List[SelectOption], placeholder: str, builds: Dict, element: str):
        ...
        for index, build in enumerate(builds):
            ...
            if has_thoughts and index+1 == len(builds):
                options.append(SelectOption(
                    label=f'{build[1]}', value=index))
            else:
                options.append(SelectOption(
                    label=f'{text_map.get(162, i.locale, user_locale)} {index+1} | {build[1]} {build[2]}', value=index))
...
class BuildSelect(Select):
    def __init__(self, options: List[SelectOption], placeholder: str, build_embeds: List[Embed]):
        super().__init__(options=options, placeholder=placeholder)
