    ...
    def __init__(self, enka_view, locale: Locale, user_locale: str | None):
        ...
        for option in self.enka_view.character_options:
            if str(option.value) == str(self.enka_view.character_id):
                continue
            teammate_options.append(SelectOption(
        ...
        for index in range(0, 3):
            self.add_item(HitModeButton(
                index, text_map.get(334+index, locale, user_locale)))
        self.add_item(ReactionModeSelect(reactionMode_options,
                      text_map.get(337, locale, user_locale)))
        options = [SelectOption(label=text_map.get(338, locale, user_locale), value='none'),
                   SelectOption(label=text_map.get(339, locale, user_locale), description=text_map.get(
                       341, locale, user_locale), value='pyro'),
                   SelectOption(label=text_map.get(340, locale, user_locale), description=text_map.get(342, locale, user_locale), value='cryo')]
        self.add_item(InfusionAuraSelect(
            options, text_map.get(343, locale, user_locale)))
        if len(teammate_options) >= 1:
            self.add_item(TeamSelect(teammate_options, text_map.get(344, locale, user_locale)))
...
class ReactionModeSelect(Select):
    ...
    async def callback(self, i: Interaction) -> Any:
        self.view: View
        self.view.calculator.reaction_mode = '' if self.values[0] == 'none' else self.values[0]
        await return_damage(i, self.view)
...
class InfusionAuraSelect(Select):
    ...
    async def callback(self, i: Interaction) -> Any:
        self.view: View
        self.view.calculator.infusion_aura = '' if self.values[0] == 'none' else self.values[0]
        await return_damage(i, self.view)
