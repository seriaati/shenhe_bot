...
class FlowGiveawayCog(commands.Cog):
                ...
                if giveaway['msgID'] == payload.message_id:
                    for user in users:
                        if user['flow'] < giveaway['ticket']:
                            await channel.send(f"{reactor.mention} 你的flow幣數量不足以參加這項抽獎", delete_after=1)
                            return
                        if user['discordID'] == payload.user_id:
                            ...
                            await channel.send(f"{reactor.mention} 花了 {giveaway['ticket']} flow幣參加 {giveaway['prize']} 抽獎", delete_after=2)
                            break
                    break
                    ...
                    for user in users:
                        if user['discordID'] == payload.user_id:
                            ...
                            newEmbed = global_vars.defaultEmbed(":tada: 抽獎啦!!!",
                                                                f"獎品: {giveaway['prize']}\n目前flow幣: {giveaway['current']}/{giveaway['goal']}\n參加抽獎要付的flow幣: {giveaway['ticket']}\n\n註: 按🎉來支付flow幣並參加抽獎\n抽獎將會在目標達到後開始")
                            await giveawayMsg.edit(embed=newEmbed)
                            await channel.send(f"{reactor.mention} 收回了 {giveaway['ticket']} flow幣來取消參加 {giveaway['prize']} 抽獎", delete_after=2)
                            break
                    break
    ...
    async def giveaway(self, ctx):
        ...
        id = uuid.uuid1()
        embed = global_vars.defaultEmbed("抽獎設置完成", ":tada:", delete_after=2)
